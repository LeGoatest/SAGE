...
*   **Legacy `src/` References**: Canonical rules (e.g., `canon/ARCHITECTURE_RULES.md`) and several skills still reference a non-existent `src/` directory for governance files.
...
*   **Rule Divergence**: `canon/ARCHITECTURE_RULES.md` (Machine) and `.docs/canon/ARCHITECTURE_RULES.md` (Human) have diverged. The machine version still cites `src/` as the authority path, while the human version is partially updated to `..docs/`.
...
*   **Pathing Drift in Skills**: Skills like `nsad`, `tailwindcss-cli`, and `doc-maintainer` contain references to legacy `src/` and `docs/` directories, violating the "No Shadow Architecture" rule (A8).
...
5.  **Unify Architecture Rules**: Synchronize the machine and human versions of `ARCHITECTURE_RULES.md` and remove all legacy references to `src/`.
